...
//        if (!TURNING) evasionManeuver();
    ...
    private void learning() {
        ...
        if (!Arrays.asList(self.getSkills()).contains(SkillType.FIREBALL)) move.setSkillToLearn(SkillType.FIREBALL);
    }
...
//            evasionManeuver();
        ...
        } else if (!canBeAttackedMoreThanOneEnemy()) { /*for one target*/
            if (getTargets().size() == 0) {
                if (getVisibleEnemies().size() == 0) {
                    goTo(getNextWaypoint(), false);
                } else {
                    goTo(selectTarget(getVisibleEnemies(), "NEAREST"), false);
                }
            } else {
                attackTarget(selectTarget(getTargets(), "NEAREST"), true);
            }
        } else { /*for several target*/
            recedeAndAttack();
        }
    }
    ...
    /**
     * Инциализируем стратегию. <p>
     * Для этих целей обычно можно использовать конструктор,
     * однако в данном случае мы хотим инициализировать генератор случайных чисел значением,
    ...
    private void initializeStrategy(Wizard self, Game game) {
        if (random == null) {
            ...
            /* Наша стратегия исходит из предположения, что заданные нами ключевые точки упорядочены по убыванию дальности до последней ключевой точки. Сейчас проверка этого факта отключена, однако вы можете написать свою проверку, если решите изменить координаты ключевых точек.*/
            /*Point2D lastWaypoint = waypoints[waypoints.length - 1];
            Preconditions.checkState(ArrayUtils.isSorted(waypoints, (waypointA, waypointB) -> Double.compare(waypointB.getDistanceTo(lastWaypoint), waypointA.getDistanceTo(lastWaypoint))));*/
        }
    }
    ...
    /**
     * Данный метод предполагает, что все ключевые точки на линии упорядочены по уменьшению дистанции до последней ключевой точки.
     * Перебирая их по порядку, находим первую попавшуюся точку, которая находится ближе к последней точке на линии, чем волшебник.
     * Это и будет следующей ключевой точкой. <p> Дополнительно проверяем, не находится ли волшебник достаточно близко к какой-либо из ключевых точек.
    ...
    private Point2D getNextWaypoint() {
        ...
        for (int waypointIndex = 0; waypointIndex < lastWaypointIndex; ++waypointIndex) {
            ...
            if (waypoint.getDistanceTo(self) <= WAYPOINT_RADIUS) return waypoints[waypointIndex + 1];
            if (lastWaypoint.getDistanceTo(waypoint) < lastWaypoint.getDistanceTo(self)) return waypoint;
        }
        return lastWaypoint;
    }

    /**
     * Действие данного метода абсолютно идентично действию метода {@code getNextWaypoint}, если перевернуть массив {@code waypoints}.
     */
    private Point2D getPreviousWaypoint() {
        ...
        for (int waypointIndex = waypoints.length - 1; waypointIndex > 0; --waypointIndex) {
            Point2D waypoint = waypoints[waypointIndex];
            if (waypoint.getDistanceTo(self) <= WAYPOINT_RADIUS) return waypoints[waypointIndex - 1];
            if (firstWaypoint.getDistanceTo(waypoint) < firstWaypoint.getDistanceTo(self)) return waypoint;
        }
    ...
    private LivingUnit selectTarget(List<LivingUnit> possibleTargets, String selectingFactor) {
        LivingUnit selectedTarget = null; /* пока будем брать ближайшую цель*/
    ...
    private boolean checkObstruction() {
        List<LivingUnit> all = new ArrayList<>();
        all.addAll(Arrays.asList(world.getMinions()));
        all.addAll(Arrays.asList(world.getWizards()));
        all.addAll(Arrays.asList(world.getBuildings()));
        all.addAll(Arrays.asList(world.getTrees()));

        for (LivingUnit object : all) {
            if (!(object instanceof Wizard && ((Wizard) object).isMe()) && ((self.getDistanceTo(object) - object.getRadius() - self.getRadius()) < 5)) {
                return true;
            }
        }
        return false;
    ...
    private void goTo(Object target, Boolean backWard) {
        ...
        if (target instanceof Point2D) {
            x = ((Point2D) target).getX();
            y = ((Point2D) target).getY();
        } else if (target instanceof LivingUnit) {
            x = ((LivingUnit) target).getX();
            y = ((LivingUnit) target).getY();
        }
        if (checkObstruction()) {
            vibration();
        }
        if (backWard) {
            ...
            if (StrictMath.abs(angle) > StrictMath.PI / 24) {
                double backAngle = StrictMath.abs(angle) > StrictMath.PI / 2 ? -(angle + StrictMath.PI - (angle > 0 ? 2 * StrictMath.PI : 0)) : angle;
                double forwardWalkingTime = angle / game.getWizardMaxTurnAngle() + self.getDistanceTo(x, y) / game.getWizardForwardSpeed();
                double strafeWalkingTime = (StrictMath.abs(angle) - StrictMath.PI / 2) / game.getWizardMaxTurnAngle() + self.getDistanceTo(x, y) / game.getWizardStrafeSpeed();
                double backwardWalkingTime = backAngle / game.getWizardMaxTurnAngle() + self.getDistanceTo(x, y) / game.getWizardBackwardSpeed();
                ...
                } else if (minTime == strafeWalkingTime) if (angle > 0) {
...
/*
    private void goTo(Point2D point) {
        ...
        if (StrictMath.abs(angle) > StrictMath.PI / 24) {
            double backAngle = StrictMath.abs(angle) > StrictMath.PI / 2 ? -(angle + StrictMath.PI - (angle > 0 ? 2 * StrictMath.PI : 0)) : angle;
            double forwardWalkingTime = angle / game.getWizardMaxTurnAngle() + self.getDistanceTo(point.getX(), point.getY()) / game.getWizardForwardSpeed();
            double strafeWalkingTime = (StrictMath.abs(angle) - StrictMath.PI / 2) / game.getWizardMaxTurnAngle() + self.getDistanceTo(point.getX(), point.getY()) / game.getWizardStrafeSpeed();
            double backwardWalkingTime = backAngle / game.getWizardMaxTurnAngle() + self.getDistanceTo(point.getX(), point.getY()) / game.getWizardBackwardSpeed();
            double minTime = StrictMath.min(StrictMath.min(forwardWalkingTime, strafeWalkingTime), backwardWalkingTime);
            if (minTime == forwardWalkingTime) {
                setTurnAdvanced(angle);
                move.setSpeed(game.getWizardForwardSpeed());
...
/*    private LivingUnit getNearestTarget() {
        ...
        *//*        double minHealth = GAME_MAX_LIFE_POINT; for (LivingUnit target : targets) { double currentLifePoint = target.getLife(); minHealth = Math.min(currentLifePoint, minHealth); targetsLife.add(target.getLife()); }*//*
        LivingUnit nearestTarget = null;
        double nearestTargetDistance = Double.MAX_VALUE;
        for (LivingUnit target : targets) {
            if (target.getFaction() == Faction.NEUTRAL || target.getFaction() == self.getFaction()) continue;
            double distance = self.getDistanceTo(target);
            if (distance < nearestTargetDistance) {
                nearestTarget = target;
                nearestTargetDistance = distance;
            } *//*  if (target.getLife() == minHealth) { nearestTarget = target; }*//*
        }
...
all.addAll(Arrays.asList(world.getTrees()));
for(
    LivingUnit livingUnit:all)

    {
        if (min > self.getDistanceTo(livingUnit) && !(livingUnit instanceof Wizard && ((Wizard) livingUnit).isMe())) {
            min = self.getDistanceTo(livingUnit);
            target = livingUnit;
        }
    }
return all;*/
